...
def test_endpoint_exists():
    ...
    with patch.dict(os.environ, {"GEMINI_API_KEY": "test_key", "INTELLIGENCE_ENABLED": "false"}):
    ...
    with patch.dict(os.environ, {"GEMINI_API_KEY": "test_key"}):
        ...
        with patch('src.api.main.ChestXRayInference', return_value=mock_pipeline):
            with patch('src.api.main.RAGRetriever', return_value=mock_retriever):
                with patch('src.api.main.GeminiSynthesizer', return_value=mock_synthesizer):
                    with patch('src.api.main.INTELLIGENCE_ENABLED', True):
                        with patch('src.api.main.PIPELINE_LOADED', True):
                            with patch('src.api.main.inference_pipeline', mock_pipeline):
                                with patch('src.api.main.rag_retriever', mock_retriever):
                                    with patch('src.api.main.gemini_synthesizer', mock_synthesizer):
                                        from src.api.main import app
...
def test_evidence_passed_to_gemini():
    """TEST 8: Evidence passed to Gemini"""
    print("\n" + "=" * 70)
    print("TEST 8: Evidence Passed to Gemini")
    print("=" * 70)
                ...
                with patch('src.api.main.gemini_synthesizer', mock_synthesizer):
                    with patch('src.api.main.INTELLIGENCE_ENABLED', True):
                        with patch('src.api.main.PIPELINE_LOADED', True):
                            ...
                            print("✓ Retrieved evidence passed to Gemini")
...
def test_gemini_response_integration():
    """TEST 9: Gemini response integration"""
    print("\n" + "=" * 70)
    print("TEST 9: Gemini Response Integration")
    print("=" * 70)
                ...
                with patch('src.api.main.gemini_synthesizer', mock_synthesizer):
                    with patch('src.api.main.INTELLIGENCE_ENABLED', True):
                        with patch('src.api.main.PIPELINE_LOADED', True):
                            ...
                            print("✓ Gemini answer included in response")
...
def run_all_tests():
    ...
    try:
        ...
        test_evidence_passed_to_gemini()
        test_gemini_response_integration()
        ...
        print(f"  ✓ Evidence passed to Gemini")
        print(f"  ✓ Gemini response integrated")
        ...
        print(f"  ✓ No fabricated answers on Gemini failure")
